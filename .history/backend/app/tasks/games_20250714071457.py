...
from sqlalchemy.orm import selectinload  # para eager load
from app import models
import io
import chess.pgn

# Herramientas de análisis detallado
from app.analysis.engine import prepare_moves_dataframe  # noqa: F401
from app.celery_app import engine_helper  # instancia global ya configurada
from app.analysis.quality import aggregate_quality_features  # noqa: F401
from app.analysis.timing import aggregate_time_features  # noqa: F401
from app.analysis.openings import aggregate_opening_features  # noqa: F401
from app.analysis.endgame import aggregate_endgame_features  # noqa: F401
from app.utils import update_progress, notify_ws  # noqa: F401
...
# ---------------------------------------------------------------------------
# Helper: cargar Game y DataFrames para análisis detallado
# ---------------------------------------------------------------------------


def _load_game_data(game_id: int, username: str):
    """Carga el registro Game y construye DataFrames necesarios.

    Devuelve tuple:
        (game_db, game_df, player_color, opening_key, eco_code, game_pgn_obj, games_df)
    """
    with Session(engine) as s:
        game_db = s.exec(
            select(models.Game)
        ...
        game_df = prepare_moves_dataframe(game_db, username)

        player_color = "white" if game_db.white_username == username else "black"

        opening_key = game_db.opening_key or ""
        eco_code = game_db.eco_code or "Unknown"

        game_pgn_obj = chess.pgn.read_game(io.StringIO(game_db.pgn))

        # DataFrame de todas las partidas del jugador (puede ser pesado)
        try:
    ...
    logger.info("DEBUG MIGRATE DETAILED: DF shapes – game_df=%s, games_df=%s", game_df.shape, games_df.shape if 'shape' in dir(games_df) else None)
...
def analyze_game_detailed(*args, **kwargs):  # noqa: D401
    """Entrada definitiva: usa implementación migrada."""
...
def _analyze_game_detailed_impl(game_id: int, username: str):
    ...
    # --- Cargar datos mínimos para futura migración -------------------
    try:
        game_db, game_df, player_color, opening_key, eco_code, game_pgn_obj, games_df = _load_game_data(game_id, username)

        # Calculamos solamente calidad y timing como prueba de migración
        q = aggregate_quality_features(game_df, elo=game_db.white_elo if player_color == 'white' else game_db.black_elo, player_color=player_color)
        t = aggregate_time_features(game_df)

        logger.info("DEBUG MIGRATE DETAILED: quality=%s", q)
        logger.info("DEBUG MIGRATE DETAILED: timing=%s", t)

        # Apertura
        try:
            o = aggregate_opening_features(opening_key, eco_code, game_df, games_df)
        except Exception as oe:
            logger.warning("DEBUG MIGRATE DETAILED: opening features falló: %s", oe)
            o = {}

        # Final (usa tablebases si existen)
        try:
            from pathlib import Path
            tb_path = TB_PATH if TB_PATH and Path(TB_PATH).exists() else None
            e = aggregate_endgame_features(game_pgn_obj, game_df, tb_path)
        except Exception as ee:
            logger.warning("DEBUG MIGRATE DETAILED: endgame features falló: %s", ee)
            e = {}

        logger.info("DEBUG MIGRATE DETAILED: opening=%s", o)
        logger.info("DEBUG MIGRATE DETAILED: endgame=%s", e)

        # Score sintético preliminar
        quality_score = q.get("quality_score", 0) or 0
        timing_score  = t.get("timing_score", 0) or 0
        opening_score = o.get("opening_score", 0) or 0
        endgame_score = e.get("endgame_score", 0) or 0

        overall_score = (
            safe(quality_score) * 0.4 +
            safe(timing_score) * 0.25 +
            safe(opening_score) * 0.2 +
            safe(endgame_score) * 0.15
        )

        logger.info("DEBUG MIGRATE DETAILED: overall_score=%s", overall_score)
    except Exception as e:
        logger.warning("DEBUG MIGRATE DETAILED: carga preliminar falló: %s", e)

    # ── Persistencia en BD (fase A) ───────────────────────────────
    from datetime import datetime, timezone

    suspicious_quality  = quality_score > 50
    suspicious_timing   = timing_score > 50
    suspicious_opening  = opening_score > 50
    suspicious_endgame  = endgame_score > 50
    ...
    detailed = models.GameAnalysisDetailed(
        ...
        ipr=q.get("ipr"),
        ipr_z_score=q.get("ipr_z_score"),
        # Tiempo
        mean_move_time=t.get("mean_move_time"),
        time_variance=t.get("time_variance"),
        ...
        suspicious_endgame=suspicious_endgame,
    ...
    cancellation_key = f"cancel:{username}"
    if redis_client.get(cancellation_key):
        logger.info("analyze_game_detailed cancelado para %s antes de commit", username)
        return {"status": "cancelled", "game_id": game_id, "username": username}

    with Session(engine) as s:
        game_check = s.get(models.Game, game_id)
        if not game_check:
            logger.warning("Game %s no existe, omitiendo guardado", game_id)
            return {"status": "cancelled", "game_id": game_id, "reason": "game_deleted"}

        s.merge(detailed)  # upsert
        s.commit()

    logger.info("DEBUG MIGRATE DETAILED: Registro GameAnalysisDetailed guardado")

    # ── Exportar a JSON para depuración y actualizar progreso ───────────
    try:
    ...
    try:
        cache_set("analyze_game_detailed", [game_id], {"username": username}, result_dict)
    except Exception:
        pass

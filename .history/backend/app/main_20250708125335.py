...
from app.middleware.apm_middleware import APMMiddleware  # middleware de APM
from prometheus_fastapi_instrumentator import Instrumentator

# Application Performance Monitoring (APM)
from app.apm import init_sentry_fastapi, set_tag, add_breadcrumb
...
# Middleware de Application Performance Monitoring (APM)
...
def analyze_game_root(request: AnalyzeGameIn, session: Session = Depends(get_session)):
    ...
    try:
        # APM: Registrar breadcrumb para análisis de partida
        add_breadcrumb(
            message="Iniciando análisis de partida",
            category="analysis",
            data={"pgn_length": len(request.pgn)}
        )
        ...
        # APM: Establecer tags para categorización
        set_tag("analysis.type", "single_game")
        set_tag("task.id", task.id)

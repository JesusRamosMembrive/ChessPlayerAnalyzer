...
def test_force_cancel():
    base_url = "http://localhost:8000"
    username = "test_force_cancel"
    
    print("=== TEST FORCE CANCEL ===")
    ...
    print(f"\n5. Esperando 10 segundos y verificando estado...")
    time.sleep(10)
    ...
    # 6. Verificar tareas activas
    print(f"\n6. Verificando tareas activas...")
    try:
        result = subprocess.run([
            sys.executable, "player_analyze_cli.py", "active"
        ], capture_output=True, text=True, cwd=".")
        
        print(f"Active tasks CLI stdout: {result.stdout}")
        print(f"Active tasks CLI stderr: {result.stderr}")
